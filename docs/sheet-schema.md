...
- `DIRECT`は送金1件以上、実装上は1件固定とする。
- `OPTIMIZED`は送金0件以上を許可する。

## 8. `transfers`

実際に行った送金ルートを保存する。提案ルートは保存しない。
...
- `DIRECT`バッチでは送金元・先が充当対象債務の債務者・債権者と一致する。
...
- `DIRECT`では、対象債務の`member_id`が送金元、元支払いの`paid_by`が送金先と一致する。
